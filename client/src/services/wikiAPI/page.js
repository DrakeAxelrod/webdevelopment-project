...
export default function wikiPage(rawPageInfo, apiOptions) {
  ...
  function mainImage() {
    return Promise.all([rawImages(), info()]).then(([images, info]) => {
      ...
      const fallback =
        image && image.imageinfo.length > 0 ? image.imageinfo[0].url : undefined
  ...
  function links(aggregated = true, limit = 100) {
    const _pagination = pagination(
      ...
      (res) =>
        (res.query.pages[raw.pageid].links || []).map((link) => link.title)
    )
    if (aggregated) {
  ...
  function rawInfo(title) {
    return api(apiOptions, {
      ...
      get(res, 'query', 'pages', firstValue, 'revisions', 0, '*')
    )
  }
